...
public class SlopeOne1
{
    ...
    public SlopeOne1(Map<String, Map<String, Double>> data, String[] AllItems)
    {
        mData = data;
        mAllItems = AllItems;
        buildDiffMatrix();
    ...
    public Map<String, Double> predict(Map<String, Double> user)
    {
        ...
        for (String j : user.keySet())
        {
            for (String k : diffMatrix.keySet())
            {
                try
                {
                    Double newval = (diffMatrix.get(k).get(j).doubleValue() + user.get(j).doubleValue()) * freqMatrix.get(k).get(j).intValue();
        ...
        for (String j : predictions.keySet())
        {
            ...
            {
                System.out.println(j +" - pre/freq => " + predictions.get(j).doubleValue() + "/" + frequencies.get(j).intValue());
                System.out.println("pre/freq => "+  predictions.get(j).doubleValue() / frequencies.get(j).intValue());
